...
exports.sendMessage = async (req, res) => {
  try {
    ...
    if (!caseId || !message) {
      return res.status(400).json({ status: 'fail', message: 'Please provide case ID and message content' });
    }
    ...
    res.status(201).json({ status: 'success', data: { messageId: result.insertId } });
  } catch (err) {
    console.error(err);
